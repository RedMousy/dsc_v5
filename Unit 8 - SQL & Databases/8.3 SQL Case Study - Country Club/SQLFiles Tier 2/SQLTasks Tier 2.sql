...
FROM country_club.Bookings AS b
	INNER JOIN country_club.Facilities AS f
	USING(facid)
...
SELECT sub.name, sub.firstname, sub.surname,
	CASE WHEN sub.firstname = 'GUEST' THEN sub.guestcost*sub.slots
	ELSE sub.membercost*sub.slots END AS cost
FROM
	(SELECT slots, membercost, guestcost, name, surname, firstname
	FROM country_club.Bookings
		INNER JOIN country_club.Facilities AS f USING(facid)
		INNER JOIN country_club.Members AS m USING(memid)
